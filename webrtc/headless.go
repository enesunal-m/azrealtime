...
func HeadlessConnect(ctx context.Context, opt HeadlessOptions) error {
    ...
    if len(opt.IceServers) > 0 { cfg.ICEServers = opt.IceServers }
    pc, err := pion.NewPeerConnection(cfg); if err != nil { return err }
    defer pc.Close()

    dc, err := pc.CreateDataChannel("realtime-channel", nil); if err != nil { return err }
    ...
    if opt.OnAudioRTP != nil {
        pc.OnTrack(func(track *pion.TrackRemote, receiver *pion.RTPReceiver) {
            ...
            for {
                ...
                if e != nil { return }
                pkts++
                if pkts%200 == 0 { opt.OnAudioRTP(pkts) }
            }
        })
    }
    ...
    if resp.StatusCode/100 != 2 { return fmt.Errorf("SDP exchange failed: %d: %s", resp.StatusCode, string(b)) }

    answer := pion.SessionDescription{ Type: pion.SDPTypeAnswer, SDP: string(b) }
    if err := pc.SetRemoteDescription(answer); err != nil { return err }
